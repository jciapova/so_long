...
/*   Updated: 2023/07/13 20:34:28 by jciapova         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_walls(t_program *start)
{
	...
	y = 0;
	i = 0;
	while (start->map_data[0][x] == '1' && start->map_data[start->map_size_y - 1][x] == '1')
		x++;
	if (x != start->map_size_x)
		ft_printf("Error: Wrong map layout\n");
	while (start->map_data[y])
	{
		if (start->map_data[y][0] == '1' && start->map_data[y][start->map_size_x - 1] == '1')
			i++;
		y++;
	}
